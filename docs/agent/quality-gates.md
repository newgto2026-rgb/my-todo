...
- 영향 범위 린트 실행
- 필요 시 루트 통합 테스트 실행
- UI/Navigation 변경 포함 시 `UI 변경 필수 검증` 항목을 추가 실행
